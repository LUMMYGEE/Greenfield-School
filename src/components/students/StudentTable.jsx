...
const StudentTable = ({
  ...
  const handleImageClick = useCallback((student) => {
    if ((userRole === 'admin' || isSuperAdmin) && student?.profileImageUrl) {
    ...
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
        ...
        <div className="flex items-center space-x-3">
          <div 
            className={`flex-shrink-0 ${
              (userRole === 'admin' || isSuperAdmin) && student?.profileImageUrl 
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-600 font-medium">
              ID: {student?.admissionNumber || "N/A"}
            </p>
      ...
      <div className="p-4">
        <div className="space-y-3">
          ...
          {student?.guardianName && (
            <div className="flex items-start space-x-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                {(student?.guardianPhone || student?.contact || student?.phoneNumber) && (
          ...
          {(student?.contact || student?.phoneNumber) && !student?.guardianName && (
            <div className="flex items-start space-x-2">
              <svg className="w-4 h-4 text-gray-400 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-medium text-gray-900">{student.contact || student.phoneNumber}</p>
              </div>
            </div>
          )}
        </div>
                    ...
                    <tr key={student?.uid || index} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                        <div 
                          ...
                          onClick={() => handleImageClick(student)}
                          title={(userRole === 'admin' || isSuperAdmin) && student?.profileImageUrl ? 'Click to view larger image' : ''}
                        >
                          <ProfileImage
                            src={student?.profileImageUrl}
                            alt={student?.fullName || "Student"}
          ...
          {actualViewMode === 'cards' && (
            <div className="p-4">
              {isMobile ? (
                <div className="space-y-4">
                  {students.map((student, index) => (
                    <MobileStudentCard key={student?.uid || index} student={student} index={index} />
                  ))}
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                  {students.map((student, index) => (
                    <DesktopStudentCard key={student?.uid || index} student={student} index={index} />
                  ))}
                </div>
              )}
            </div>
